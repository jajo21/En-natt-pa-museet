                                ...
                                foreach (var museum in museumStorage.GetMuseumDictionary())
                                {
                                    if (museumStorage.GetVisitingMuseumName() == museum.Key)
                                    {
                                        foreach (var room in museum.Value.GetRoomList())
                                            if (options[1] == room.GetRoomNameString())
                                            {
                                ...
                                if (museumStorage.GetVisitingMuseumName() == museum.Key)
                                {
                                    foreach (var room in museum.Value.GetRoomList())
                                    {
                                        if (options[1] == room.GetRoomNameString())
                                        {
                                            try
                                            {
                                                room.GetArtList().Remove(room.GetArtList()[Convert.ToInt32(options[2]) - 1]);
                                                Console.WriteLine($"Konstverket {room.GetArtList()[Convert.ToInt32(options[2]) - 1]} är nu borttaget");
                                            }
                                            catch
                                            {
